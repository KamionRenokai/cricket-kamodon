...
BGM(0) = bgmfldr$ + "TechDemoTheme.mp3"
...
SEF(0) = _SNDOPEN(sndfldr$ + "FlameWareLogo.ogg", "VOL,SYNC")
SEN(0) = "FlameWare Zeronexe Logo -- Xmuzik"
SEF(1) = _SNDOPEN(sndfldr$ + "CKJump.ogg", "VOL,SYNC")
...
SEF(2) = _SNDOPEN(sndfldr$ + "StereoEcho1UP.ogg", "VOL,SYNC")
SEN(2) = "Cricket Gets a 1UP -- www.freesfx.co.uk"
SEF(3) = _SNDOPEN(sndfldr$ + "CKDeathSymphony.ogg", "VOL,SYNC")
SEN(3) = "Cricket Dies -- L. von Beethoven, Musopen"
SEF(4) = _SNDOPEN(sndfldr$ + "CKVertMalletSwing.ogg", "VOL,SYNC")
...
SEF(8) = _SNDOPEN(sndfldr$ + "CKHorizMalletSwing.ogg", "VOL,SYNC")
...
SEF(13) = _SNDOPEN(sndfldr$ + "Alert3Pause.ogg", "VOL,SYNC")
SEN(13) = "Pause or Unpause the Game -- www.freesfx.co.uk"
SEF(14) = _SNDOPEN(sndfldr$ + "ClockTick.ogg", "VOL,SYNC")
SEN(14) = "Remaining Time Converted to Points"
SEF(15) = _SNDOPEN(sndfldr$ + "TimeRunningOut.ogg", "VOL,SYNC")
SEN(15) = "Time is Running Out"
SEF(16) = _SNDOPEN(sndfldr$ + "TimeUpBuzzer.ogg", "VOL,SYNC")
SEN(16) = "Out of Time"
ckHurt& = _SNDOPEN(sndfldr$ + "SQPauseSound.ogg", "VOL,SYNC")
...
IF _FILEEXISTS("WLDXL1LD.NRT") THEN
    OPEN "WLDXL1LD.NRT" FOR INPUT AS #5
...
ELSE
    COLOR 12, 0: PRINT "FAIL! I can't find WLDXL1LD.NRT!"
    END
END IF

COLOR 7, 0: PRINT "FG1 layer (1/3)? "; 'Using the NRT files for right now.
IF _FILEEXISTS("WLDXL1F1.NXT") THEN
    OPEN "WLDXL1F1.NXT" FOR INPUT AS #1
    DO UNTIL EOF(1)
    ...
    END IF
ELSE
    COLOR 12, 0: PRINT "FAIL! I can't find WLDXL1F1.NXT!"
    END
END IF

COLOR 7, 0: PRINT "FG2 layer (1/3)? "; 'Using the NRT files for right now.
IF _FILEEXISTS("WLDXL1F2.NXT") THEN
    OPEN "WLDXL1F2.NXT" FOR INPUT AS #2
    DO UNTIL EOF(2)
    ...
    END IF
ELSE
    COLOR 12, 0: PRINT "FAIL! I can't find WLDXL1F2.NXT!"
    END
END IF

COLOR 7, 0: PRINT "BG1 layer (1/3)? "; 'Using the NRT files for right now.
IF _FILEEXISTS("WLDXL1B1.NXT") THEN
    OPEN "WLDXL1B1.NXT" FOR INPUT AS #3
    DO UNTIL EOF(3)
    ...
    END IF
ELSE
    COLOR 12, 0: PRINT "FAIL! I can't find WLDXL1B1.NXT!"
    END
END IF

COLOR 7, 0: PRINT "BG2 layer (1/3)? "; 'Using the NRT files for right now.
IF _FILEEXISTS("WLDXL1B2.NXT") THEN
    OPEN "WLDXL1B2.NXT" FOR INPUT AS #4
    DO UNTIL EOF(4)
    ...
    END IF
ELSE
    COLOR 12, 0: PRINT "FAIL! I can't find WLDXL1B2.NXT!"
    END
END IF

COLOR 7, 0: PRINT "Level info all set up (2/3)? ";
IF _FILEEXISTS("WLDXL2LD.KMD") THEN
    OPEN "WLDXL2LD.KMD" FOR INPUT AS #10
...
IF _FILEEXISTS("WLDXL2F1.KMD") THEN
    OPEN "WLDXL2F1.KMD" FOR INPUT AS #6
    DO UNTIL EOF(6)
...
IF _FILEEXISTS("WLDXL2F2.KMD") THEN
    OPEN "WLDXL2F2.KMD" FOR INPUT AS #7
    DO UNTIL EOF(7)
...
IF _FILEEXISTS("WLDXL2B1.KMD") THEN
    OPEN "WLDXL2B1.KMD" FOR INPUT AS #8
    DO UNTIL EOF(8)
...
IF _FILEEXISTS("WLDXL2B2.KMD") THEN
    OPEN "WLDXL2B2.KMD" FOR INPUT AS #9
    DO UNTIL EOF(9)
...
IF _FILEEXISTS("WLDXL3LD.KMD") THEN
    OPEN "WLDXL3LD.KMD" FOR INPUT AS #15
...
IF _FILEEXISTS("WLDXL3F1.KMD") THEN
    OPEN "WLDXL3F1.KMD" FOR INPUT AS #11
    DO UNTIL EOF(11)
...
IF _FILEEXISTS("WLDXL3F2.KMD") THEN
    OPEN "WLDXL3F2.KMD" FOR INPUT AS #12
    DO UNTIL EOF(12)
...
IF _FILEEXISTS("WLDXL3B1.KMD") THEN
    OPEN "WLDXL3B1.KMD" FOR INPUT AS #13
    DO UNTIL EOF(13)
...
IF _FILEEXISTS("WLDXL3B2.KMD") THEN
    OPEN "WLDXL3B2.KMD" FOR INPUT AS #14
    DO UNTIL EOF(14)
...
destpath$ = chrfldr$ + "KTE-Stand0.png"
CrickMov(0) = _LOADIMAGE(destpath$)
FOR ws = 0 TO 7
    destpath$ = chrfldr$ + "KTE-Walk" + LTRIM$(STR$(ws)) + ".png"
    CrickMov(ws + 1) = _LOADIMAGE(destpath$)
NEXT ws
destpath$ = chrfldr$ + "KTE-Jump0.png"
jump& = _LOADIMAGE(destpath$)
destpath$ = chrfldr$ + "KTE-Climb0.png"
...
DO
    ...
    'TODO: Prevent the game from crashing if one of the background or foreground levels calls for a tile that isn't loaded.
    FOR y = 0 TO 28
        FOR x = 0 TO 41 '(tsc - 1)
...
IF _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
	...
	END IF
        IF CKL% = 148 AND sprnum(0) > -1 AND stl < 27 THEN 'Scroll the background
            ...
            END IF
            'ELSEIF CKL% < 148 THEN
        ELSEIF sprnum(0) = -1 AND sprpos(0) = -8 OR stl = 27 OR str = 27 THEN 'Move the player
...
ELSEIF _KEYDOWN(MoveRight) AND slidecount% = 0 THEN
	...
	str = 0 'This checks to see if we can scroll the screen.
        ...
        'BUG: Running into a wall that's near a STOP tile freezes your character in place.
        'Case-in-point: the wall at the right-most end of the lowest level in Subcon 1, near the beanstalk.
            CKL% = CKL% + 1: CKR% = CKR% + 1: CKX% = CKX% + 1
            IF plyr& = CrickMov(mf) THEN mt = mt + 1
            IF mt = 12 THEN
                mt = 0: mf = mf + 1: IF mf = 8 THEN mf = 1
...
FOR tc = 0 TO tilecnt%
    ...
    destpath$ = sprfldr$ + LTRIM$(STR$(tc)) + ext$
    spritedata(tc) = _LOADIMAGE(destpath$)
NEXT tc
        ...
        CASE 13 ' ENTER
            SELECT CASE Highlight 'A tiny bit of code optimization, here.
                ...
                CASE 4 'Start > Subcon 1
                    ...
                    LevelData$ = "WLDXL1LD.NRT"
                    Foreground1$ = "WLDXL1F1.NXT"
                    Foreground2$ = "WLDXL1F2.NXT"
                    Background1$ = "WLDXL1B1.NXT"
                    Background2$ = "WLDXL1B2.NXT"
                    OPEN LevelData$ FOR INPUT AS #5
                    OPEN Foreground1$ FOR INPUT AS #1
                    OPEN Foreground2$ FOR INPUT AS #2
                    OPEN Background1$ FOR INPUT AS #3
                ...
                CASE 5 'Start > Subcon 2
                    ...
                    IF actbgm% >= 0 AND bgm& THEN _SNDSTOP bgm&
                    zone$ = "SUBCON 2"
                    LevelData$ = "WLDXL2LD.KMD"
                    Foreground1$ = "WLDXL2F1.KMD"
                    Foreground2$ = "WLDXL2F2.KMD"
                    Background1$ = "WLDXL2B1.KMD"
                    Background2$ = "WLDXL2B2.KMD"
                    OPEN LevelData$ FOR INPUT AS #5
                    OPEN Foreground1$ FOR INPUT AS #1
                    OPEN Foreground2$ FOR INPUT AS #2
                    OPEN Background1$ FOR INPUT AS #3
                ...
                CASE 6 'Start > Subcon 3
                    ...
                    zone$ = "SUBCON 3"
                    IF actbgm% >= 0 AND bgm& THEN _SNDSTOP bgm&
                    LevelData$ = "WLDXL3LD.KMD"
                    Foreground1$ = "WLDXL3F1.KMD"
                    Foreground2$ = "WLDXL3F2.KMD"
                    Background1$ = "WLDXL3B1.KMD"
                    Background2$ = "WLDXL3B2.KMD"
                    OPEN LevelData$ FOR INPUT AS #5
                    OPEN Foreground1$ FOR INPUT AS #1
                    OPEN Foreground2$ FOR INPUT AS #2
                    OPEN Background1$ FOR INPUT AS #3
