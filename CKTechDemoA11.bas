...
tilecnt% = 0
respath$ = "/media/PHANTOM/QBX/CRICKET/" '<-- Remember to EDIT anything using this, when "leaking" alpha builds.
sprfldr$ = "SPRITES/"
...
DO
    ...
    'Debug and state information, such as character positioning (TODO: Make it so you can toggle this with a key)
    ...
    IF WhatIsTime# >= .1 THEN
        csc = csc + 1
        IF csc = 10 THEN fps = fc: fc = 0
        IF csc = 10 AND tick > 0 THEN GOSUB ClockChange: csc = 0
...
ELSEIF _KEYDOWN(MoveJump) AND jdrop = 0 AND jpush = 0 AND slidecount% = 0 THEN 'Holding down the JUMP key
    IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
    IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
    IF jump% < 42 THEN 'Height test, instead of setting the coordinate.
        notop = 0
