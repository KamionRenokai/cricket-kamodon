...
_TITLE "Cricket Kamodon - alpha 12 Tech Demo"
SCREEN 12: COLOR 7
PRINT "ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป"
PRINT "บ CRICKET KAMODON!         ** GAME ENGINE TECH DEMO **        12th Alpha Build บ"
...
tilecnt% = 0
respath$ = "Z:\media\PHANTOM\QBX\CRICKET\" '<-- Remember to EDIT anything using this, before "leaking" alpha builds.
sprfldr$ = "SPRITES\"
...
DO
            ...
            IF PRow < 27 THEN 'Bugfix attempt, so it doesn't try to check what's below Cricket's feet, if he fell off the screen, on the lowest screen level.
                IF LevelData(vert% + PRow, UnderFoot(Foot)) = 1 THEN 'Platform
                    ...
                    IF cont% = 0 AND jpush = 1 THEN
                        IF NOT _KEYDOWN(MoveJump) THEN jpush = 0: mf = 0: plyr& = CrickMov(mf): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1: IF CKB% MOD 8 = 0 THEN CKT% = CKT% - 1: CKB% = CKB% - 1
                    ELSEIF cont% > 0 AND jpush = 1 THEN
                        IF JoyMoveOp(5) > 0 AND MoveJump < 128 THEN
                            IF STICK(ActionJump, JoyMoveOp(5)) > MoveJump THEN jpush = 0: mf = 0: plyr& = CrickMov(mf): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1: IF CKB% MOD 8 = 0 THEN CKT% = CKT% - 1: CKB% = CKB% - 1
                        ELSEIF JoyMoveOp(5) > 0 AND MoveJump > 128 THEN
                            IF STICK(ActionJump, JoyMoveOp(5)) < MoveJump THEN jpush = 0: mf = 0: plyr& = CrickMov(mf): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1: IF CKB% MOD 8 = 0 THEN CKT% = CKT% - 1: CKB% = CKB% - 1
                        ELSEIF JoyMoveOp(5) = 0 AND STRIG(MoveJump, ActionJump) = 0 THEN jpush = 0: mf = 0: plyr& = CrickMov(mf): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1: IF CKB% MOD 8 = 0 THEN CKT% = CKT% - 1: CKB% = CKB% - 1
                        END IF
                    END IF                
    ...
    'Debug and state information, such as character positioning -- TODO: make it so you can turn the DEBUG info off if you want to!
    ...
    fc = fc + 1
    LET WhatIsTime# = ClockCheck# - TimeIsNow#
    IF WhatIsTime# >= .1 THEN
        csc = csc + 1
        f = f + 1: IF f = 16 THEN f = 0
        IF csc = 10 THEN 
			fps = fc: fc = 0
			IF tick > 0 THEN GOSUB ClockChange
			csc = 0
		END IF
...
IF _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
    ...
    IF (vert% + PRow) > 0 AND PRow < 27 AND CKT% > 10 THEN
        noblock = 0
...
' Jump (by itself)
'********************
IF slidecount% = 0 AND jdrop = 0 AND jpush = 0 THEN
    IF JoyMoveOp(5) > 0 AND MoveJump < 128 THEN
...
IF JoyMoveOp(6) > 0 AND MoveAttack < 128 THEN
    IF STICK(ActionAttack, JoyMoveOp(6)) < MoveAttack THEN
        ...
        END IF
        IF ma THEN _SNDPLAY SEF(18) ELSE _SNDPLAY SEF(4)
        ma = 0
    END IF
...
IF jdrop THEN 'If you let off the JUMP key
    IF jpush = 0 THEN jpush = 1
    IF rj = 1 THEN rj = 0
    IF plyr& <> CrickMov(0) THEN plyr& = CrickMov(0): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
    CKT% = CKT% + 1 '   This brings the player sprite back down to the
...
DRAW "B D10"
DRAW "C" + STR$(&HFF009900)
Font "ENGINE TECH DEMO ALPHA 12"
DRAW "B L209"
DRAW "B D35"
DRAW "C" + STR$(&HFF000000)
Font "COPYRIGHT 2011-2013 FLAMEWARE ZERONEXE"
