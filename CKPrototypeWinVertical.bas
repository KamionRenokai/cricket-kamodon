...
' Copyright 2011-2013 Robbie Bruce a.k.a. Kamion R. Shanoriko
'
...
ELSE 'If it is, then let's DIM them properly
	...
	DIM SHARED MnS(atm - 1, 0 TO 5) AS INTEGER 'Monster number, left/right/top/bottom/global coords/spawn status
...
FOR d = 0 TO (adm - 1)
	INPUT #6, a$, b$, c$ 'For testing purposes, the first one should read "000", "ENEMY" and "ShyGuy.png".
...
FOR t = 0 TO (atm - 1) 'Set up each enemy (work-in-progress)
	INPUT #7, j$, k$, l$, m$ 'For testing purposes, the first one should read "000", "R", "130" and "18".
...
'**** Change for alpha 12: Bye-bye "ClockChange", hello tenth-of-a-second-to-the-clock timing
...
DO
    ...
    FOR y = 0 TO 28
        FOR x = 0 TO 41 '(tsc - 1)
            ...
            '**** Change for alpha 12: thanks to "_CLIP", I can use PUT in the same way I used _PUTIMAGE in alphas 9 through 11
	...
	FOR r = 0 TO (atm - 1) 'Place any monsters that were just spawned, or are already active, on the screen
		IF MnS(r, 5) = 1 THEN 'Bugfix: the "invisible curtain that slowly sweeps to the left, engulfing monsters" is history.
	...
	IF fs = 5 THEN 'Bugfix: all other enemies keep moving, even after one has stopped, for whatever reason
		FOR ai = 0 TO (atm - 1) 'See the comment above the monster spawning routine for a bug that should be fixed.
			IF slidecount% = 0 AND MnS(ai, 5) = 1 THEN
				'IF MnS(ai, 0) > 0 THEN '<-- I don't think moving where this is checked broke anything...
...
' Move Left
'********************
IF atk = -1 AND _KEYDOWN(MoveLeft) THEN 'AND slidecount% = 0 THEN 'I should probably re-implement running, sometime...
...
ELSEIF kp& = MoveAttack THEN 'Pressing the ATTACK key by itself
	IF atk = -1 THEN atk = 0: last& = plyr&: fr = CKR% 'This starts the attack animation sequence, and saves the pose we were in
...
'** KEYPRESS CHECKING SUBROUTINE -> GAMEPAD (This needs a few tweaks, but actually works great, so far!)
JoyCtrlCheck:
'********************
' Move Left
'********************
...
ELSEIF JoyMoveOp(5) > 0 AND MoveJump > 128 THEN 'AND slidecount% = 0 THEN
    IF STICK(ActionJump, JoyMoveOp(5)) < MoveJump AND jump% > 0 THEN jdrop = 1: jpush = 1
ELSEIF JoyMoveOp(5) = 0 THEN 'I hope this fixes that bug... I think it did, if I can remember...
