...
_TITLE "Cricket Kamodon - v0.1.5 Tech Demo"
PRINT "ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป"
PRINT "บ CRICKET KAMODON!         ** GAME ENGINE TECH DEMO **           Version 0.1.5 บ"
...
'255 per row with a one thousand sprite palette.
'TODO: Throw an error if _FILEEXISTS turns out to be false for any of these
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDYL1FG.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDYL1FG.KMD" FOR INPUT AS #1 'Mushroom 1 foreground
...
'      Also, change WLDYL1FG.KMD's name back to WLDXL1FG.KMD, after testing!
OPEN "/media/PHANTOM/QBX/CRICKET/WLDYL1FG.KMD" FOR INPUT AS #1
OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1B1.KMD" FOR INPUT AS #2
...
PRINT "will definitely have a completely different level arrangement, with more levels."
PRINT
...
lc% = (grab / 22) / 255 'How many times does the total go into 256?
rc% = (grab / 22) MOD 255 'Is there a remainder? (One extra loop if there is)

IF lc% < 1 THEN 'If the total is less than 255... run through one loop.
    FOR r% = 0 TO 21
...
ELSEIF lc% >= 1 THEN '       If the total is at least 255 or more, loop as
    FOR l% = 0 TO (lc% - 1) 'many times as the total number of tiles per row
        FOR r% = 0 TO 21 '   goes into 255.
            FOR s% = 0 TO 254
                INPUT #1, ns$ 'Grab one of the foreground layer sprites
                Foreground1(r%, s% + (255 * l%)) = VAL(ns$) 'Transform!
                ns$ = "" 'Erase what it's set to, just in case
                INPUT #2, ns$ 'Go for the first background layer, now
                Background1(r%, s% + (255 * l%)) = VAL(ns$) 'Transform!
                ns$ = "" 'Erase it again, Tony
                INPUT #3, ns$ 'Take something from background layer two!
                Background2(r%, s% + (255 * l%)) = VAL(ns$) 'Transform!
            NEXT s%
        NEXT r%
    NEXT l%

    'Now, if there was at least 255 or more, but still some left afterward,
    'let's run through one more loop, to get the remaining sprites.

    IF rc% > 0 THEN 'Any sprites left after reading each multiple of 255?
        FOR r% = 0 TO 21
            FOR s% = 0 TO (((grab / 22) - (255 * lc%)) - 1)
                INPUT #1, ns$ 'Grab a sprite
                Foreground1(r%, ((255 * lc%) + s%)) = VAL(ns$) 'Save it
                ns$ = "" 'Clear the variable, because I'm paranoid
                INPUT #2, ns$ 'Grab another sprite
                Background1(r%, ((255 * lc%) + s%)) = VAL(ns$) 'Save it
                ns$ = "" 'Clear that thing! Get it far away from me! lol
                INPUT #3, ns$ 'Grab yet another sprite
                Background2(r%, ((255 * lc%) + s%)) = VAL(ns$) 'Save it
            NEXT s%
        NEXT r%
    END IF
...
DO
    ...
    FOR screenY = 24 TO 199
        ...
        FOR screenX = 1 TO 320
            ...
            IF spritedata(Background2(rowpos%, sprpos%), 0, spiteElem) > 0 THEN
                POKE offset&, spritedata(Background2(rowpos%, sprpos%), 0, spriteElem)
                'PSET (screenX, screenY), spritedata(Background2(rowpos%, sprpos%), 0, spriteElem)
            END IF
    ...
    IF _KEYDOWN(19200) THEN 'Left arrow key
        ...
        IF _KEYDOWN(122) THEN 'Scroll the screen faster if Z is held, too.
            ...
            END IF
        END IF
        IF _KEYDOWN(120) THEN _SNDPLAY (ckSwing1&)
        IF _KEYDOWN(99) THEN _SNDPLAY (ckSwing1&)
