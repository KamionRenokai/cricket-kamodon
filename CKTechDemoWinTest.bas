...
tilecnt% = 0
respath$ = "Z:\media\PHANTOM\QBX\CRICKET\"
sprfldr$ = "SPRITES\"
...
IF _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
    ...
    IF _KEYDOWN(MoveJump) AND jdrop = 0 AND jpush = 0 THEN 'Holding down JUMP
        ...
        IF _KEYDOWN(MoveRun) THEN 'The player jumps higher if it's running!
            IF jump% < 60 THEN
                notop = 0
                IF CKT% <= 8 THEN 'Bugfix. If nothing's above him, he just jumps.
...
ELSEIF _KEYDOWN(MoveRight) AND slidecount% = 0 THEN
    ...
    IF _KEYDOWN(MoveJump) AND jdrop = 0 AND jpush = 0 THEN 'Holding down JUMP
        ...
        ELSE
            IF jump% < 42 THEN
                notop = 0
                IF CKT% <= 1 THEN 'Bugfix. If nothing's above him, he just jumps.
                    CKT% = CKT% - 1: CKB% = CKB% - 1: jump% = jump% + 1
                ELSEIF CKT% > 1 THEN
                    FOR Head = 0 TO sn
                        IF LevelData(vert% + (PRow - 5), AboveHead(Head)) = 2 THEN
...
ELSEIF _KEYDOWN(MoveJump) AND jdrop = 0 AND jpush = 0 AND slidecount% = 0 THEN
    ...
    IF jump% < 42 THEN 'Height test, instead of setting the coordinate.
        notop = 0
        IF CKT% <= 9 THEN 'Bugfix. If nothing's above him, he just jumps.
            CKT% = CKT% - 1: CKB% = CKB% - 1: jump% = jump% + 1
        ELSEIF CKT% > 9 THEN
            FOR Head = 0 TO sn
                IF LevelData(vert% + (PRow - 5), AboveHead(Head)) = 2 THEN
...
IF _KEYDOWN(MoveDReload) AND _KEYDOWN(MoveDReset) THEN 'CTRL+R reloads all level data
    IF actbgm% >= 0 AND bgm& THEN _SNDPAUSE bgm&
    _SNDPLAY SEF(13)
    TIMER OFF
    _AUTODISPLAY
...
' Move Left
'********************
IF JoyMoveOp(0) > 0 AND STICK(ActionUp, JoyMoveOp(0)) = MoveUp OR JoyMoveOp(0) = 0 AND STRIG(MoveUp, ActionUp) AND slidecount% = 0 THEN
    'IF _KEYDOWN(122) THEN 'Hold down Z
    '    FOR q = 0 TO (tsc - 1)
    '        sprpos(q) = sprpos(q) + 2
    '    NEXT q
...
IF JoyMoveOp(11) > 0 AND STICK(ActionDReload, JoyMoveOp(11)) = MoveDReload OR JoyMoveOp(11) = 0 AND STRIG(MoveDReload, ActionDReload) AND JoyMoveOp(9) > 0 AND STICK(ActionDReset, JoyMoveOp(9)) = MoveDReset OR JoyMoveOp(9) = 0 AND STRIG(MoveDReset, ActionDReset) THEN
    IF actbgm% >= 0 AND bgm& THEN _SNDPAUSE bgm&
    _SNDPLAY SEF(13)
    TIMER OFF
    _AUTODISPLAY
            ...
            SELECT CASE Highlight 'A tiny bit of code optimization, here.
                ...
                CASE 5 'Start > Subcon 2
                    ...
                    FOR sf = 0 TO (msn - 1) 'Get each song filename.
                        ...
                        BGM(sf) = sf$ 'the level data, where we need it.
                    NEXT sf
                    BGM(0) = "Z:\media\PHANTOM\IMPULSE\SOTRFALT.IT"
                    BGM(1) = respath$ + bgmfldr$ + "SubUnderworld.mp3"
                    BGM(2) = respath$ + bgmfldr$ + "Birdo.mp3"
                    IF actbgm% >= 0 THEN bgm& = _SNDOPEN(BGM(0), "VOL,PAUSE")
                    EXIT DO
