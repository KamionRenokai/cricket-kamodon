...
ON TIMER(0.1) GOSUB ClockChange ' Call the subroutine once every second to
...
IF _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
    ...
    IF noblock > 3 AND CKX% > 0 THEN 'If nothing's in our way, let's move!
        ...
        IF CKL% = 148 AND sprnum(0) > -1 AND stl < 27 THEN 'Scroll the background
            ...
            END IF
            'ELSEIF CKL% < 148 THEN
        ELSEIF CKL% > 0 AND CKL% < 148 OR sprnum(0) = -1 AND sprpos(0) = -8 OR stl = 27 OR str = 27 THEN 'Move the player
...
'** KEYPRESS CHECKING SUBROUTINE -> GAMEPAD (Experimental, PLEASE test this!)
JoyCtrlCheck:
'********************
' Move Left
'********************
