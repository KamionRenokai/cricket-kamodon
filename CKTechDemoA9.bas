...
' These will be removed, in favor of a new fade in/out routine.
...
COMMON SHARED CKL%, CKR%, CKT%, CKB%, CKX%, cont%, gp%
...
IF dev > 2 THEN 'The first two are the keyboard and mouse.
    ...
    COLOR 15, 0: PRINT "YES! Found" + STR$(gp%) + "!"
    ...
    NEXT j
ELSEIF dev <= 2 THEN
    COLOR 15, 0: PRINT "NO! But that's okay!"
...
IF _FILEEXISTS(respath$ + "WLDXL1LD.KMD") THEN
    OPEN respath$ + "WLDXL1LD.KMD" FOR INPUT AS #5
    INPUT #5, lvname$ 'First, we get the name of the level.
...
IF _FILEEXISTS(respath$ + "WLDXL1F1.NRT") THEN
    OPEN respath$ + "WLDXL1F1.NRT" FOR INPUT AS #1
    DO UNTIL EOF(1)
...
IF _FILEEXISTS(respath$ + "WLDXL1F2.NRT") THEN
    OPEN respath$ + "WLDXL1F2.NRT" FOR INPUT AS #2
    DO UNTIL EOF(2)
...
IF _FILEEXISTS(respath$ + "WLDXL1B1.NRT") THEN
    OPEN respath$ + "WLDXL1B1.NRT" FOR INPUT AS #3
    DO UNTIL EOF(3)
...
IF _FILEEXISTS(respath$ + "WLDXL1B2.NRT") THEN
    OPEN respath$ + "WLDXL1B1.NRT" FOR INPUT AS #4
    DO UNTIL EOF(4)
    ...
    IF lc% < 1 THEN 'If the total is less than 256... loop once.
        FOR r% = 0 TO 21
            FOR s% = 0 TO (((grab& / 22) / scrcnt%) - 1)
                LOCATE 4, 1: PRINT "DEBUG: Grabbing row" + STR$((r% + (22 * sl%)) + 1) + ", tile" + STR$(s% + 1) + " A"
    ...
    ELSEIF lc% >= 1 THEN '          If the total is at least 256 or more,
        FOR l% = 0 TO (lc% - 1) '   loop as many times as the total number
            FOR r% = 0 TO 21 '      of tiles per row goes into 256.
                FOR s% = 0 TO 255
                    LOCATE 4, 1: PRINT "DEBUG: Grabbing row" + STR$((r% + (22 * sl%)) + 1) + ", tile" + STR$(256 * l% + (s% + 1)) + " B"
        ...
        IF rc% > 0 THEN 'Any sprites left after reading each multiple of 256?
            FOR r% = 0 TO 21
                FOR s% = 0 TO ((((grab& / 22) / scrcnt%) - (256 * lc%)) - 1)
                    LOCATE 4, 1: PRINT "DEBUG: Grabbing row" + STR$((r% + (22 * sl%)) + 1) + ", tile" + STR$((256 * lc%) + s% + 1) + " BC"
...
DIM SHARED verrow(0 TO 23) AS INTEGER 'The Y coordinate of each row of sprites
FOR n = 0 TO 41
    sprseg(n) = 8
    sprnum(n) = (n - 1)
    sprpos(n) = ((n - 1) * 8)
...
DO
    CLS 'For the debug HUD, until I raise the number of playfield rows to 27.
    ...
    FOR y = 0 TO 23
        FOR x = 0 TO 41 '(tsc - 1)
            ...
            IF sprnum(x) >= 0 AND verrow(y) < 176 THEN
                IF y = 0 AND (vert% - 1) = -1 THEN ELSE _PUTIMAGE (sprpos(x), 64 + verrow(y)), spritedata(Background1((vert% - 1) + y, sprnum(x)))
                IF y = 0 AND (vert% - 1) = -1 THEN ELSE _PUTIMAGE (sprpos(x), 64 + verrow(y)), spritedata(Background2((vert% - 1) + y, sprnum(x)))
                IF y = 0 AND (vert% - 1) = -1 THEN ELSE _PUTIMAGE (sprpos(x), 64 + verrow(y)), spritedata(Foreground1((vert% - 1) + y, sprnum(x)))
                IF y = 0 AND (vert% - 1) = -1 THEN ELSE _PUTIMAGE (sprpos(x), 64 + verrow(y)), spritedata(Foreground2((vert% - 1) + y, sprnum(x)))
            END IF
        NEXT x
    NEXT y
    ...
    FOR Foot = 0 TO (sn - 1) 'The number of sprites below Cricket's feet
        IF LevelData(vert% + PRow, UnderFoot(Foot)) = 1 THEN 'Platform
            ...
            IF NOT _KEYDOWN(120) AND jpush = 1 THEN jpush = 0
    ...
    IF CKT% >= 241 AND slidecount% = 0 AND (vert% + 22) / 22 = scrcnt% THEN
    ...
    ELSEIF CKT% >= 236 AND slidecount% = 0 AND (vert% + 22) / 22 <> scrcnt% THEN
        'But if he didn't, we'll scroll the screen up!
        slidecount% = 176
        'jdrop = 0
        CKT% = CKT% - 1: CKB% = (CKT% + _HEIGHT(plyr&) - 1)
    ELSEIF CKB% <= 64 AND slidecount% = 0 AND (vert% + 22) / 22 > 0 THEN
    ...
    IF _KEYDOWN(19200) AND slidecount% = 0 THEN
        ...
        IF _KEYDOWN(120) AND jdrop = 0 AND jpush = 0 THEN 'Holding down JUMP
            ...
            IF _KEYDOWN(122) THEN 'The player jumps higher if it's running!
                IF jump% < 60 THEN
                    notop = 0
                    IF CKT% <= 70 THEN 'Bugfix. If nothing's above him, he just jumps.
    ...
    ELSEIF _KEYDOWN(18432) AND slidecount% = 0 THEN
        ...
        FOR cc = 0 TO sn
            IF LevelData(vert% + (PRow - 1), AboveHead(cc)) = 3 THEN
    ...
    SELECT CASE _KEYHIT 'Works with the keyboard repeat rate, unlike _KEYDOWN.
        ...
        CASE 13 ' ENTER
            SELECT CASE Highlight 'A tiny bit of code optimization, here.
                ...
                CASE 4 'Start > Subcon 1
                    ...
                    OPEN respath$ + "WLDXL1F2.NRT" FOR INPUT AS #2
                    OPEN respath$ + "WLDXL1B1.NRT" FOR INPUT AS #3
                    OPEN respath$ + "WLDXL1B2.NRT" FOR INPUT AS #4
        ...
        CASE 27 'ESC
            ...
            ELSEIF Quadrant = 3 THEN 'Reconfigure Controls menu
                ...
                DRAW "B M1,115 C" + STR$(&HFFFFFFFF)
                Font "BACKGROUND MUSIC"
                DRAW "B M155,115 C" + STR$(&HFF00FF00)
                Font "YES, PLEASE!"
                DRAW "B M1,125 C" + STR$(&HFFFFFFFF)
                Font "SOUND TEST"
                DRAW "B M1,135 C" + STR$(&HFFFFFFFF)
...
'TODO: Integrate this with FadeOut, if I can, so the music fades in/out, too
IF actbgm% >= 0 AND bgm& THEN
    FOR v = 1 TO 0 STEP 0.1
        _SNDVOL bgm&, v
    NEXT v
...
FOR b = 255 TO 0 STEP -1
    _LIMIT 190 'Control the speed (good for fast PCs; how about slower ones?)
