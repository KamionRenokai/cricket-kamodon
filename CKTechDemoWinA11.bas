...
'** GLOBAL VARIABLES
COMMON SHARED zone$, score&, lives, amp%, tick, scrcnt%, vert%, actbgm%, bgm&, prevbgm%
COMMON SHARED ckHurt&, respath$, sprfldr$, chrfldr$, bgmfldr$, sndfldr$, ext$
COMMON SHARED CKL%, CKR%, CKT%, CKB%, CKX%, cont%, gp%, pn, lvname$, msn, ct, res
...
_TITLE "Cricket Kamodon - alpha 11 Tech Demo"
SCREEN 12: COLOR 7
PRINT "ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป"
PRINT "บ CRICKET KAMODON!         ** GAME ENGINE TECH DEMO **        11th Alpha Build บ"
...
NEXT ws
destpath$ = respath$ + chrfldr$ + "JC-Jump0.png"
...
DO
    ...
    IF jdrop THEN 'Only check what's under Cricket's feet if he's not jumping
        FOR Foot = 0 TO (sn - 1) 'The number of sprites below Cricket's feet
            IF PRow < 27 THEN 'Bugfix attempt, so it doesn't try to check what's below Cricket's feet, if he fell off the screen, on the lowest screen level.
    ...
    IF CKT% > 240 AND slidecount% = 0 AND (vert% + 27) / 27 = scrcnt% THEN
        ...
        IF actbgm% >= 0 AND _SNDPLAYING(bgm&) THEN _SNDSTOP bgm& 'Stop the background music
    ...
    ELSEIF NOT pf THEN
        _PUTIMAGE (CKL%, CKT%), plyr& 'Let your conscience face right!
    END IF
    ...
    ELSEIF kp& = MoveDScrlDn AND slidecount% = 0 THEN 'PLUS key scrolls the screen down 27 lines (DEBUG)
        IF vert% < ((27 * scrcnt%) - 27) THEN
            slidecount% = 216
        END IF

    ELSEIF kp& = MoveDScrlUp AND slidecount% = 0 THEN 'MINUS key scrolls the screen up 27 lines (DEBUG)
        IF vert% > 0 THEN
            slidecount% = -216
        END IF
    ...
    ELSEIF _KEYDOWN(MoveDReload) AND _KEYDOWN(MoveDReset) THEN GOSUB LevelReload
...
IF _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
    ...
    IF _KEYDOWN(MoveJump) AND jdrop = 0 AND jpush = 0 THEN 'Holding down JUMP
        ...
        ELSE
            IF jump% < 42 THEN
                notop = 0
                IF CKT% <= 18 THEN 'BUG: Jumping off the screen at the topmost screen level still crashes the game.
                    CKT% = CKT% - 1: CKB% = CKB% - 1: jump% = jump% + 1
                ELSEIF CKT% > 18 THEN
                    FOR Head = 0 TO sn
                        IF LevelData(vert% + (PRow - 5), AboveHead(Head)) = 2 THEN
    ...
    IF (vert% + PRow) > 0 AND PRow < 27 AND CKT% > 1 THEN
        noblock = 0
...
'** KEYPRESS CHECKING SUBROUTINE -> GAMEPAD (This segment is a rework-in-progress!)
JoyCtrlCheck:
'********************
' Move Left
'********************
...
IF slidecount% = 0 AND STICK(ActionLeft, JoyMoveOp(2)) = MoveLeft THEN
    'IF _KEYDOWN(122) THEN 'Hold down Z
    '    FOR q = 0 TO (tsc - 1)
    '        sprpos(q) = sprpos(q) + 2
    '    NEXT q
	...
	IF jdrop = 0 AND jpush = 0 AND STRIG(MoveJump, ActionJump) THEN
        IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
		...
		IF STRIG(MoveRun, ActionRun) THEN
            IF jump% < 60 THEN
                notop = 0
                IF CKT% <= 10 THEN 'BUG: Jumping off the screen at the topmost screen level still crashes the game.
                    CKT% = CKT% - 1: CKB% = CKB% - 1: jump% = jump% + 1
        ...
        END IF
    END IF
    IF STRIG((MoveJump - 1), ActionJump) THEN jdrop = 1: jpush = 1 'Let go of the JUMP key early
    IF jdrop THEN 'When you let go of JUMP
        IF jpush = 0 THEN jpush = 1
        IF plyr& <> CrickMov(0) THEN plyr& = CrickMov(0): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        CKT% = CKT% + 1
	...
	IF STRIG(MoveAttack, ActionAttack) THEN _SNDPLAY SEF(8)
    'Check for collisions, before we decide if we can move, or not
...
ELSEIF slidecount% = 0 AND STICK(ActionRight, JoyMoveOp(3)) = MoveRight THEN
    'IF _KEYDOWN(122) THEN 'Hold down Z
    '    FOR q = 0 TO (tsc - 1)
    '        sprpos(q) = sprpos(q) - 2
    '    NEXT q
    'ELSE
    IF jdrop = 0 AND jpush = 0 AND STRIG(MoveJump, ActionJump) THEN 'Holding down JUMP
        IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        IF STRIG(MoveRun, ActionRun) THEN 'The player jumps higher if it's running!
            'Maybe copy this routine to the UP and DOWN arrow routines?
            IF jump% < 60 THEN
                notop = 0
                IF CKT% <= 10 THEN 'BUG: Jumping off the screen at the topmost screen level still crashes the game.
...
ELSEIF slidecount% = 0 AND STICK(ActionUp, JoyMoveOp(0)) = MoveUp THEN
    climb = 0
    FOR cc = 0 TO sn
        IF LevelData(vert% + (PRow - 3), AboveHead(cc)) = 3 AND LevelData(vert% + (PRow - 2), AboveHead(cc)) = 3 THEN
            climb = climb + 1 'If Cricket can climb what's above him
	...
	IF jump% > -2 AND jdrop = 0 AND jpush = 0 AND STRIG(MoveJump, ActionJump) THEN
        'IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        IF jump% < 42 THEN 'Height test, instead of setting the coordinate.
            notop = 0
        ...
        ELSE
            ...
            jdrop = 1
        END IF
    ELSEIF STRIG((MoveJump - 1), ActionJump) THEN jdrop = 1: jpush = 1 'Let go of JUMP key early
    ELSEIF jdrop THEN 'If you let off the JUMP key
        IF jpush = 0 THEN jpush = 1
        IF plyr& <> CrickMov(0) THEN plyr& = CrickMov(0): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        CKT% = CKT% + 1 '   This brings the player sprite back down to the
...
ELSEIF slidecount% = 0 AND STICK(ActionDown, JoyMoveOp(1)) = MoveDown THEN
    climb = 0
    FOR cc = 0 TO sn
        IF LevelData(vert% + (PRow - 2), AboveHead(cc)) = 3 AND LevelData(vert% + (PRow - 1), AboveHead(cc)) = 3 THEN
            climb = climb + 1 'If Cricket can climb what's below him
	...
	IF jump% > -2 AND jdrop = 0 AND jpush = 0 AND STRIG(MoveJump, ActionJump) THEN
        ...
        ELSE
            ...
            jdrop = 1
        END IF
    ELSEIF STRIG((MoveJump - 1), ActionJump) THEN jdrop = 1: jpush = 1 'Let go of the JUMP key early
    ELSEIF jdrop THEN 'If you let off the JUMP key
        IF jpush = 0 THEN jpush = 1
        IF plyr& <> CrickMov(0) THEN plyr& = CrickMov(0): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        CKT% = CKT% + 1 '   This brings the player sprite back down to the
...
ELSEIF STRIG(MovePause, ActionPause) THEN
    IF actbgm% >= 0 AND bgm& THEN _SNDPAUSE bgm&
    _SNDPLAY SEF(13)
    TIMER OFF
    GOSUB PauseGame
...
ELSEIF STRIG(MoveRun, ActionRun) THEN
    _SNDPLAY ckHurt& 'Just for testing purposes.
    'I wanna get the movement speed down, first. Then I'll make this work.
...
ELSEIF jdrop = 0 AND jpush = 0 AND slidecount% = 0 AND STRIG(MoveJump, ActionJump) THEN
    IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
    IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
    IF jump% < 42 THEN 'Height test, instead of setting the coordinate.
        notop = 0
...
ELSEIF STRIG(MoveAttack, ActionAttack) THEN
    _SNDPLAY SEF(4) 'Just for testing purposes.
    'I wonder if I should make it so you can hold down the attack key?
    'END IF

    '** Change for alpha 11: the DEBUG keys are permanently mapped to the keyboard, which, to me, makes sense.
END IF
...
'        PSET (mx + 1, my), _RGBA32(0, 0, 0, 127)
'        PSET (mx, my + 1), _RGBA32(0, 0, 0, 127)
'        PSET (mx + 1, my + 1), _RGBA32(0, 0, 0, 255)
'    NEXT mx
'NEXT my

'LINE (0, 53)-(320, 188), _RGBA32(0, 0, 0, 140), BF
    ...
    IF WhatIsTime! >= .01 THEN
        ...
        IF Sector = 0 THEN 'The main pause menu.
            ...
            ELSEIF cont% > 0 AND JoyMoveOp(7) = 0 THEN
                CenterFont "PRESS ''BUTTON" + STR$(MovePause) + "'' AGAIN TO UNPAUSE", 85
            END IF
        ...
        ELSEIF Sector = 1 THEN 'The "Controls" menu.
            ...
            DRAW "B M50,105 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveUp)
            DRAW "B M50,115 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveDown)
            DRAW "B M50,125 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveLeft)
            DRAW "B M50,135 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveRight)
            DRAW "B M50,145 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveRun)
            DRAW "B M50,155 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveJump)
            DRAW "B M50,165 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MoveAttack)
            DRAW "B M50,175 C" + STR$(&HFF00FF00): IF cont% = 0 THEN Font ButtonDef$(MovePause)
        ...
        'Initially skipping "LOAD" and "SAVE" for the tech demo only.
        ...
        _DISPLAY
    END IF
    IF cont% = 0 THEN 'You get this set of CASE statements if you're using the keyboard.
        SELECT CASE _KEYHIT 'Works with the keyboard repeat rate, unlike _KEYDOWN.
                ...
                SELECT CASE Selector 'A tiny bit of code optimization, here. (which I totally copied from CricketMenu)
                    ...
                    CASE 3 'Music Toggle (and later ambience toggle, too)
                        ...
                        IF actbgm% >= 0 THEN
                            ...
                            _SNDCLOSE bgm&
                            _PUTIMAGE (0, 0)-(319, 239), ps&
                            'DRAW "C" + STR$(&HFF000000): CenterFont "MUSIC IS ON", 145
                            'DRAW "C" + STR$(&HFFFFFFFF): CenterFont "MUSIC IS OFF", 145
                        ELSEIF actbgm% = -1 THEN
                            actbgm% = prevbgm%
                            bgm& = _SNDOPEN(BGM(actbgm%), "VOL,PAUSE")
                            _PUTIMAGE (0, 0)-(319, 239), ps&
                            'DRAW "C" + STR$(&HFF000000): CenterFont "MUSIC IS OFF", 145
                            'DRAW "C" + STR$(&HFFFFFFFF): CenterFont "MUSIC IS ON", 145
                            '_SNDLOOP bgm&
                        END IF
                    CASE 4 'Exit to Menu
                        _SNDPLAY SEF(4)
                    ...
                    CASE 6 'Control Scheme Preset
                        ...
                        END IF
                        _PUTIMAGE (0, 0)-(319, 239), ps&
                    CASE 7 'Remap Up (case 7-14 all repeat mostly the same code, as I can't GOSUB from a subroutine.)
                        _SNDPLAY SEF(8)
                        kc& = _KEYHIT: IF kc& = 13 THEN DO: kc& = _KEYHIT: LOOP UNTIL kc& <> 13 'Stop holding down the ENTER key!
                        DO: kg& = _KEYHIT: LOOP WHILE kg& <= 0 'We don't want negative numbers. You get one when you let off a key.
                    ...
                    CASE 15 'Yes! I'm positive!
                        ...
                        GOTO LoopBack 'Return to the main menu (can I, from a subroutine, without messing things up?)
            ...
            CASE MovePause 'Press the PAUSE key again
                EXIT DO
        END SELECT
    ELSEIF cont% > 0 THEN 'Or you get this set if you're using a gamepad.
    END IF
LOOP
    ...
    SELECT CASE _KEYHIT 'Works with the keyboard repeat rate, unlike _KEYDOWN.
        ...
        CASE 13 ' ENTER
            SELECT CASE Highlight 'A tiny bit of code optimization, here.
                ...
                CASE 4 'Start > Subcon 1 (should put the overworld map in, instead of a level select menu?)
                    _SNDPLAY SEF(4)
                    IF actbgm% >= 0 AND bgm& THEN _SNDSTOP bgm&
                    zone$ = "SUBCON 1"
                ...
                CASE 12 'Options > Reconfigure Controls > Control Scheme Preset
                    ...
                    DRAW "B M50,165 C" + STR$(&HFF00FF00)
                    IF cont% = 0 THEN Font ButtonDef$(MovePause) ELSE Font GamepadDef$(7)
                CASE 13: IF cont% = 0 THEN GOSUB Remap
                CASE 14: IF cont% = 0 THEN GOSUB Remap
                CASE 15: IF cont% = 0 THEN GOSUB Remap
                CASE 16: IF cont% = 0 THEN GOSUB Remap
                CASE 17: IF cont% = 0 THEN GOSUB Remap
                CASE 18: IF cont% = 0 THEN GOSUB Remap
                CASE 19: IF cont% = 0 THEN GOSUB Remap
                CASE 20: IF cont% = 0 THEN GOSUB Remap
            END SELECT
        CASE 27 'ESC
...
elseif cont% > 0 then
	...
	do
		kg& = _keyhit 'For gamepad checking, this just checks to see if the user presses ESC, instead.
		if kg& = 27 then _SNDPLAY SEF(1): exit do 'Stop checking the active gamepad if ESC is pressed
		xc = cont% * 2
		sax = stick(xc - 2, 1)
		if sax = 254 or sax = 1 then exit do
		say = stick(xc - 1, 1)
		if say = 254 or say = 1 then exit do
		sbx = stick(xc - 2, 2)
		if sbx = 254 or sbx = 1 then exit do
		sby = stick(xc - 1, 2)
		if sby = 254 or sby = 1 then exit do
		dpx = stick(xc - 2, 3)
		if dpx = 254 or dpx = 1 then exit do
		dpy = stick(xc - 1, 3)
		if dpy = 254 or dpy = 1 then exit do
	...
	IF Highlight = 13 THEN
        IF sax = 254 OR sax = 1 THEN MoveUp = sax: ActionUp = xc - 2: JoyMoveOp(0) = 1
        IF say = 254 OR say = 1 THEN MoveUp = say: ActionUp = xc - 1: JoyMoveOp(0) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveUp = sbx: ActionUp = xc - 2: JoyMoveOp(0) = 2
        IF sby = 254 OR sby = 1 THEN MoveUp = sby: ActionUp = xc - 1: JoyMoveOp(0) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveUp = dpx: ActionUp = xc - 2: JoyMoveOp(0) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveUp = dpy: ActionUp = xc - 1: JoyMoveOp(0) = 3
        IF bn > 0 THEN MoveUp = bn: ActionUp = cont%
    ELSEIF Highlight = 14 THEN
        IF sax = 254 OR sax = 1 THEN MoveDown = sax: ActionDown = xc - 2: JoyMoveOp(1) = 1
        IF say = 254 OR say = 1 THEN MoveDown = say: ActionDown = xc = 1: JoyMoveOp(1) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveDown = sbx: ActionDown = xc - 2: JoyMoveOp(1) = 2
        IF sby = 254 OR sby = 1 THEN MoveDown = sby: ActionDown = xc - 1: JoyMoveOp(1) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveDown = dpx: ActionDown = xc - 2: JoyMoveOp(1) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveDown = dpy: ActionDown = xc - 1: JoyMoveOp(1) = 3
        IF bn > 0 THEN MoveDown = bn: ActionDown = cont%
    ELSEIF Highlight = 15 THEN
        IF sax = 254 OR sax = 1 THEN MoveLeft = sax: ActionLeft = xc - 2: JoyMoveOp(2) = 1
        IF say = 254 OR say = 1 THEN MoveLeft = say: ActionLeft = xc - 1: JoyMoveOp(2) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveLeft = sbx: ActionLeft = xc - 2: JoyMoveOp(2) = 2
        IF sby = 254 OR sby = 1 THEN MoveLeft = sby: ActionLeft = xc - 1: JoyMoveOp(2) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveLeft = dpx: ActionLeft = xc - 2: JoyMoveOp(2) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveLeft = dpy: ActionLeft = xc - 1: JoyMoveOp(2) = 3
        IF bn > 0 THEN MoveLeft = bn: ActionLeft = cont%
    ELSEIF Highlight = 16 THEN
        IF sax = 254 OR sax = 1 THEN MoveRight = sax: ActionRight = xc - 2: JoyMoveOp(3) = 1
        IF say = 254 OR say = 1 THEN MoveRight = say: ActionRight = xc - 1: JoyMoveOp(3) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveRight = sbx: ActionRight = xc - 2: JoyMoveOp(3) = 2
        IF sby = 254 OR sby = 1 THEN MoveRight = sby: ActionRight = xc - 1: JoyMoveOp(3) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveRight = dpx: ActionRight = xc - 2: JoyMoveOp(3) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveRight = dpy: ActionRight = xc - 1: JoyMoveOp(3) = 3
        IF bn > 0 THEN MoveRight = bn: ActionRight = cont%
    ELSEIF Highlight = 17 THEN
        IF sax = 254 OR sax = 1 THEN MoveRun = sax: ActionRun = xc - 2: JoyMoveOp(4) = 1
        IF say = 254 OR say = 1 THEN MoveRun = say: ActionRun = xc - 1: JoyMoveOp(4) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveRun = sbx: ActionRun = xc - 2: JoyMoveOp(4) = 2
        IF sby = 254 OR sby = 1 THEN MoveRun = sby: ActionRun = xc - 1: JoyMoveOp(4) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveRun = dpx: ActionRun = xc - 2: JoyMoveOp(4) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveRun = dpy: ActionRun = xc - 1: JoyMoveOp(4) = 3
        IF bn > 0 THEN MoveRun = bn: ActionRun = cont%
    ELSEIF Highlight = 18 THEN
        IF sax = 254 OR sax = 1 THEN MoveJump = sax: ActionJump = xc - 2: JoyMoveOp(5) = 1
        IF say = 254 OR say = 1 THEN MoveJump = say: ActionJump = xc - 1: JoyMoveOp(5) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveJump = sbx: ActionJump = xc - 2: JoyMoveOp(5) = 2
        IF sby = 254 OR sby = 1 THEN MoveJump = sby: ActionJump = xc - 1: JoyMoveOp(5) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveJump = dpx: ActionJump = xc - 2: JoyMoveOp(5) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveJump = dpy: ActionJump = xc - 1: JoyMoveOp(5) = 3
        IF bn > 0 THEN MoveJump = bn: ActionJump = cont%
    ELSEIF Highlight = 19 THEN
        IF sax = 254 OR sax = 1 THEN MoveAttack = sax: ActionAttack = xc - 2: JoyMoveOp(6) = 1
        IF say = 254 OR say = 1 THEN MoveAttack = say: ActionAttack = xc - 1: JoyMoveOp(6) = 1
        IF sbx = 254 OR sbx = 1 THEN MoveAttack = sbx: ActionAttack = xc - 2: JoyMoveOp(6) = 2
        IF sby = 254 OR sby = 1 THEN MoveAttack = sby: ActionAttack = xc - 1: JoyMoveOp(6) = 2
        IF dpx = 254 OR dpx = 1 THEN MoveAttack = dpx: ActionAttack = xc - 2: JoyMoveOp(6) = 3
        IF dpy = 254 OR dpy = 1 THEN MoveAttack = dpy: ActionAttack = xc - 1: JoyMoveOp(6) = 3
        IF bn > 0 THEN MoveAttack = bn: ActionAttack = cont%
    ELSEIF Highlight = 20 THEN
        IF sax = 254 OR sax = 1 THEN MovePause = sax: ActionPause = xc - 2: JoyMoveOp(7) = 1
        IF say = 254 OR say = 1 THEN MovePause = say: ActionPause = xc - 1: JoyMoveOp(7) = 1
        IF sbx = 254 OR sbx = 1 THEN MovePause = sbx: ActionPause = xc - 2: JoyMoveOp(7) = 2
        IF sby = 254 OR sby = 1 THEN MovePause = sby: ActionPause = xc - 1: JoyMoveOp(7) = 2
        IF dpx = 254 OR dpx = 1 THEN MovePause = dpx: ActionPause = xc - 2: JoyMoveOp(7) = 3
        IF dpy = 254 OR dpy = 1 THEN MovePause = dpy: ActionPause = xc - 1: JoyMoveOp(7) = 3
...
DRAW "B D10"
DRAW "C" + STR$(&HFF009900)
Font "ENGINE TECH DEMO ALPHA 11"
DRAW "B L209"
DRAW "B D35"
DRAW "C" + STR$(&HFF000000)
Font "COPYRIGHT 2011-2013 FLAMEWARE ZERONEXE"
...
SELECT CASE btncode
    ...
    CASE 100311: ButtonDef$ = "LEFT WINDOWS LOGO" ' The Linux build calls this the "Super" key. Will the OS X build call it "Command"?
    CASE 100312: ButtonDef$ = "RIGHT WINDOWS LOGO" 'When you press this key on Piccolo, it doesn't register as this code.
    ...
    CASE 0 'Direction "up"
        ...
        ELSEIF JoyMoveOp(0) > 0 THEN
            ...
            IF ActionUp MOD 2 = 0 THEN
                IF MoveUp = 1 THEN at$ = at$ + " LEFT"
                IF MoveUp = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionUp MOD 2 = 1 THEN
                IF MoveUp = 1 THEN at$ = at$ + " UP"
                IF MoveUp = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$
        END IF
    CASE 1 'Direction "down"
        ...
        ELSEIF JoyMoveOp(1) > 0 THEN
            ...
            IF ActionDown MOD 2 = 0 THEN
                IF MoveDown = 1 THEN at$ = at$ + " LEFT"
                IF MoveDown = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionDown MOD 2 = 1 THEN
                IF MoveDown = 1 THEN at$ = at$ + " UP"
                IF MoveDown = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$
        END IF
    CASE 2 'Direction "left"
        ...
        ELSEIF JoyMoveOp(2) > 0 THEN
            ...
            IF ActionLeft MOD 2 = 0 THEN
                IF MoveLeft = 1 THEN at$ = at$ + " LEFT"
                IF MoveLeft = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionLeft MOD 2 = 1 THEN
                IF MoveLeft = 1 THEN at$ = at$ + " UP"
                IF MoveLeft = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$
        END IF
    CASE 3 'Direction "right"
        ...
        ELSEIF JoyMoveOp(3) > 0 THEN
            ...
            IF ActionRight MOD 2 = 0 THEN
                IF MoveRight = 1 THEN at$ = at$ + " LEFT"
                IF MoveRight = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionRight MOD 2 = 1 THEN
                IF MoveRight = 1 THEN at$ = at$ + " UP"
                IF MoveRight = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$			
        END IF
    CASE 4 'Action "run"
        ...
        ELSEIF JoyMoveOp(4) > 0 THEN
            ...
            IF ActionRun MOD 2 = 0 THEN
                IF MoveRun = 1 THEN at$ = at$ + " LEFT"
                IF MoveRun = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionRun MOD 2 = 1 THEN
                IF MoveRun = 1 THEN at$ = at$ + " UP"
                IF MoveRun = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$			
        END IF
    CASE 5 'Action "jump"
        ...
        ELSEIF JoyMoveOp(5) > 0 THEN
            ...
            IF ActionJump MOD 2 = 0 THEN
                IF MoveJump = 1 THEN at$ = at$ + " LEFT"
                IF MoveJump = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionJump MOD 2 = 1 THEN
                IF MoveJump = 1 THEN at$ = at$ + " UP"
                IF MoveJump = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$
        END IF
    CASE 6 'Action "attack"
        ...
        ELSEIF JoyMoveOp(6) > 0 THEN
            ...
            IF ActionAttack MOD 2 = 0 THEN
                IF MoveAttack = 1 THEN at$ = at$ + " LEFT"
                IF MoveAttack = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionAttack MOD 2 = 1 THEN
                IF MoveAttack = 1 THEN at$ = at$ + " UP"
                IF MoveAttack = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$			
        END IF
    CASE 7 'Action "pause"
        ...
        ELSEIF JoyMoveOp(7) > 0 THEN
            ...
            IF ActionPause MOD 2 = 0 THEN
                IF MovePause = 1 THEN at$ = at$ + " LEFT"
                IF MovePause = 254 THEN at$ = at$ + " RIGHT"
            ELSEIF ActionPause MOD 2 = 1 THEN
                IF MovePause = 1 THEN at$ = at$ + " UP"
                IF MovePause = 254 THEN at$ = at$ + " DOWN"
            END IF
            GamepadDef$ = at$			
        END IF
END SELECT
