...
'This i where I should move the loading routines into their own subroutine, like I tried with the last two versions.
...
FOR sl% = 0 TO (scrcnt% - 1)
    ...
    IF lc% < 1 THEN 'If the total is less than 256... loop once.
        FOR r% = 0 TO 26
            FOR s% = 0 TO (((backA& / 27) / scrcnt%) - 1)
                LOCATE 10, 1: PRINT "Reading row" + STR$((r% + (27 * sl%)) + 1) + ", tile" + STR$(s% + 1) + " A"
    ...
    ELSEIF lc% >= 1 THEN '          If the total is at least 256 or more, loop as many times as the total number
        FOR l% = 0 TO (lc% - 1) '   of tiles per row goes into 256.
            FOR r% = 0 TO 26
                FOR s% = 0 TO 255
                    LOCATE 10, 1: PRINT "Reading row" + STR$((r% + (27 * sl%)) + 1) + ", tile" + STR$(256 * l% + (s% + 1)) + " B"
        ...
        IF rc% > 0 THEN 'Any sprites left after reading each multiple of 256?
            FOR r% = 0 TO 26
                FOR s% = 0 TO ((((backA& / 27) / scrcnt%) - (256 * lc%)) - 1)
                    LOCATE 10, 1: PRINT "Reading row" + STR$((r% + (27 * sl%)) + 1) + ", tile" + STR$((256 * lc%) + s% + 1) + " BC"
