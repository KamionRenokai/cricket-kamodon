...
PRINT "บ CRICKET KAMODON!         ** GAME ENGINE TECH DEMO **         8th Alpha Build บ"
...
BGM(0) = "/media/PHANTOM/QBX/CRICKET/MUSIC/TechDemoTheme.mp3"
...
SEF(0) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/FlameWareLogo.ogg", "VOL,SYNC")
SEN(0) = "FlameWare Logo Theme -- Xmuzik"
SEF(1) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/CKJump.ogg", "VOL,SYNC")
...
SEF(2) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/GFSNESBell.ogg", "VOL,SYNC")
SEN(2) = "Cricket Gets a 1UP -- Temporary"
SEF(3) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/CKDeathSymphony.ogg", "VOL,SYNC")
SEN(3) = "Cricket Dies -- L. von Beethoven, Musopen"
SEF(4) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/CKVertMalletSwing.ogg", "VOL,SYNC")
...
SEF(8) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/CKHorizMalletSwing.ogg", "VOL,SYNC")
...
SEF(13) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/Alert3Pause.ogg", "VOL,SYNC")
SEN(13) = "Pause or Unpause the Game -- freesfx.co.uk"
SEF(14) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/ClockTick.ogg", "VOL,SYNC")
SEN(14) = "Remaining Time Converted to Points"
SEF(15) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/TimeRunningOut.ogg", "VOL,SYNC")
SEN(15) = "Time is Running Out"
SEF(16) = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/TimeUpBuzzer.ogg", "VOL,SYNC")
SEN(16) = "Out of Time"
ckHurt& = _SNDOPEN("/media/PHANTOM/QBX/CRICKET/SOUND/SQPauseSound.ogg", "VOL,SYNC")
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1LD.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1LD.KMD" FOR INPUT AS #6
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1F1.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1F1.KMD" FOR INPUT AS #1
    DO UNTIL EOF(1)
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1F2.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1F2.KMD" FOR INPUT AS #2
    DO UNTIL EOF(2)
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1B2.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1B1.KMD" FOR INPUT AS #4
    DO UNTIL EOF(4)
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1S0.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1S0.KMD" FOR INPUT AS #5
    DO UNTIL EOF(5)
        FOR n = 0 TO 63
...
CKB% = 55 '      Coordinate of the very bottom of Cricket's feet
...
DO
    ...
    FOR screenY = 24 TO 199
        ...
        FOR screenX = 1 TO 320
            ...
            '      the sprites that's on either BG2, FG1 or FG2 is color "0",
            ...
            'PSET ((screenX - 1), screenY), spritedata(Background1(vert% + rowpos%, sprpos%), 0, spriteElem)
            ...
            IF spritedata(Background2(vert% + rowpos%, sprpos%), 0, spriteElem) > 0 THEN
                ...
                'PSET ((screenX - 1), screenY), spritedata(Background2(vert% + rowpos%, sprpos%), 0, spriteElem)
            END IF
            ...
            IF spritedata(Foreground1(vert% + rowpos%, sprpos%), 0, spriteElem) > 0 THEN
                ...
                'PSET ((screenX - 1), screenY), spritedata(Foreground1(vert% + rowpos%, sprpos%), 0, spriteElem)
            END IF
            ...
            IF spritedata(Foreground2(vert% + rowpos%, sprpos%), 0, spriteElem) > 0 THEN
                ...
                'PSET ((screenX - 1), screenY), spritedata(Foreground2(vert% + rowpos%, sprpos%), 0, spriteElem)
            END IF
            offset& = offset& + 1
    ...
    IF CKT% >= 201 AND CKB% >= 232 THEN 'Check his coordinates
        IF (vert% + 22) / 22 = scrcnt% THEN 'Bottom screen? That's a death.
    ...
    IF _KEYDOWN(19200) AND slidecount% = 0 THEN 'Left arrow key
        ...
        IF _KEYDOWN(120) AND jdrop = 0 THEN 'Holding down JUMP
            ...
            IF jump% = 0 THEN _SNDPLAY SEF(1) 'Only play the sound once.
        ...
        IF jdrop THEN 'When you let go of JUMP
            ...
            CKB% = CKB% + 3
        END IF
        IF _KEYDOWN(99) THEN _SNDPLAY SEF(8) ' Holding down ATTACK
    ...
    ELSEIF _KEYDOWN(19712) AND slidecount% = 0 THEN 'Right arrow key
        ...
        IF hScroll <= ((((grab& / 22) / scrcnt%) * 8) - 320) AND NOT _KEYDOWN(122) THEN
            ...
            END IF
        END IF
    ELSEIF _KEYDOWN(18432) AND slidecount% = 0 THEN 'Up arrow key
        IF _KEYDOWN(122) THEN 'Manually scroll if the RUN key is held (DEBUG)
            'If we're not already on the highest screen, let's scroll up once.
            'Should the screen scroll pixel by pixel, instead of tile by tile?
            IF vert% > 0 THEN
                vert% = vert% - 1
                slidecount% = -21
            END IF
        ELSE
            climb = 0 'BUG: You freeze in midair if you jump, then hold UP.
            FOR cc = 0 TO sn
                IF LevelData(vert% + (PRow - 3), AboveHead(cc)) = 3 THEN
                    climb = climb + 1 'If Cricket can climb what's above him
                END IF
            NEXT cc 'If he's in front of two climbable tiles, let's climb up!
            IF climb = 2 THEN CKT% = CKT% - 2: CKB% = CKB% - 2: jump% = -2
        END IF
    ELSEIF _KEYDOWN(20480) AND slidecount% = 0 THEN 'Down arrow key
        ...
        IF _KEYDOWN(122) THEN 'Scroll ONLY if the RUN key is held (DEBUG)
            IF vert% < ((22 * scrcnt%) - 22) THEN
                vert% = vert% + 1
                slidecount% = 21
            END IF
        ELSE
            climb = 0 'Check this code, because Cricket isn't sliding anymore
            FOR cc = 0 TO sn
                IF LevelData(vert% + (PRow - 2), AboveHead(cc)) = 3 THEN
                    climb = climb + 1 'If Cricket can climb what's below him
                ELSEIF LevelData(vert% + PRow, AboveHead(cc)) = 1 OR 2 THEN
    ...
    ELSEIF kp& = 114 THEN 'Pressing R resets Cricket's position (DEBUG key)
        ...
        jdrop = 1
        jump% = 0
        IF NOT _SNDPLAYING(bgm&) THEN _SNDLOOP bgm&
    ...
    ELSEIF _KEYDOWN(120) AND jdrop = 0 THEN 'Hold down the JUMP key by itself
        IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1) 'Only play the JUMP sound once.
        IF jump% < 14 THEN 'Height test, instead of setting the coordinate.
            notop = 0
            IF CKT% <= 32 THEN 'Bugfix. If nothing's above him, he just jumps.
                CKT% = CKT% - 3: CKB% = CKB% - 3: jump% = jump% + 1
            ELSEIF CKT% > 32 THEN
        ...
        CASE 13 ' ENTER
            SELECT CASE Highlight 'A tiny bit of code optimization, here.
                ...
                CASE 4 'Start > Subcon 1
                    ...
                    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1F2.KMD" FOR INPUT AS #2
                    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1B1.KMD" FOR INPUT AS #3
                    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1B2.KMD" FOR INPUT AS #4
                    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1S0.KMD" FOR INPUT AS #5
                    ...
                    IF bgm& THEN _SNDCLOSE bgm&
                    'Soon, this will also be added to the level data file, as
                    'well as any additional sound effects each level uses, so
                    'they can be dynamically loaded in and out, per level.
                    BGM(0) = "/media/PHANTOM/IMPULSE/SOTRFALT.IT"
                    BGM(1) = "/media/PHANTOM/QBX/CRICKET/MUSIC/SubUnderworld.mp3"
                    BGM(2) = "/media/PHANTOM/QBX/CRICKET/MUSIC/Birdo.mp3"
                    bgm& = _SNDOPEN(BGM(0), "VOL,PAUSE")
                    EXIT DO
                ...
                CASE 10 'Options > Sound Test
                    _SNDPLAY SEF(4)
                    IF bgm& THEN _SNDPAUSE bgm& 'Moved here. Better spot.
                    LINE (0, 59)-(320, 148), 0, BF 'Erase the menu options.
                    DRAW "B M119,65 C15"
                    Font "SOUND TEST"
                    DRAW "B M1,76"
