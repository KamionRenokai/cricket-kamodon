...
COMMON SHARED CKL%, CKR%, CKT%, CKB%, CKX%, cont%, gp%, pn, lvname$, msn, ct, res, atk
...
tilecnt% = 0
respath$ = "/media/Lexar/PROJECTS/CRICKET/" '<-- Remember to EDIT anything using this, before "leaking" alpha builds.
sprfldr$ = "SPRITES/"
...
IF res THEN 'Check: is this the first time, this run, that we've DIM'd these arrays?
    ...
    REDIM SHARED MonsterAnim(0, 0 TO 3, 0) AS LONG 'Animation type: 0=walk, 1=attack, 2=hurt, 3=death (? WIP)
ELSE 'If it is, then let's DIM them properly
    ...
    DIM SHARED MonsterAnim(0, 0 TO 3, 0) AS LONG 'Number order: monster number, animation type, animation frame.
END IF
...
DO
    ...
    IF pf THEN 'I WAS using Knuckles the Echidna to test this, but my brother suggested someone different...
        _PUTIMAGE (CKR%, CKT%)-(CKL%, CKB%), plyr&, small 'Let your conscience face left!
    ELSEIF NOT pf THEN
        _PUTIMAGE (CKL%, CKT%), plyr&, small 'Let your conscience face right!
    END IF
...
' Move Left
'********************
IF atk = -1 AND _KEYDOWN(MoveLeft) AND slidecount% = 0 THEN
