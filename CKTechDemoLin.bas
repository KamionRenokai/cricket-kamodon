...
IF dev > 2 THEN 'The first two are the keyboard and mouse.
    ...
    FOR j = 3 TO dev 'Now we need to find out the name of each gamepad.
        ...
        IF (j - 2) MOD 2 = 0 THEN 'Even-numbered gamepads (2 or 4)
            ...
            DefKeys(j - 2, 1, 0, 0) = 19: DefKeys(j - 2, 1, 1, 0) = 0: DefKeys(j - 2, 1, 2, 0) = 0
            DefKeys(j - 2, 1, 0, 1) = 27: DefKeys(j - 2, 1, 1, 1) = 0: DefKeys(j - 2, 1, 2, 1) = 0
            DefKeys(j - 2, 1, 0, 2) = 31: DefKeys(j - 2, 1, 1, 2) = 0: DefKeys(j - 2, 1, 2, 2) = 0
            DefKeys(j - 2, 1, 0, 3) = 23: DefKeys(j - 2, 1, 1, 3) = 0: DefKeys(j - 2, 1, 2, 3) = 0
...
FOR tc = 0 TO tilecnt%
    LOCATE 5, 1: PRINT "Loading sprite " + LTRIM$(STR$(tc)) + "... ";
...
IF slidecount% = 0 AND STICK(ActionLeft, JoyMoveOp(2)) = MoveLeft THEN
    'IF JoyMoveOp(2) > 0 THEN IF STICK(ActionLeft, JoyMoveOp(2)) = MoveLeft AND slidecount% = 0 THEN
    'OR JoyMoveOp(2) = 0 AND STRIG(MoveLeft, ActionLeft) AND slidecount% = 0 THEN
    'IF _KEYDOWN(122) THEN 'Hold down Z
    '    FOR q = 0 TO (tsc - 1)
    '        sprpos(q) = sprpos(q) + 2
    '    NEXT q
    ...
    IF jpush = 0 AND jdrop = 0 AND STRIG(MoveJump, ActionJump) THEN
        IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        ...
        END IF
    END IF
    IF STRIG((MoveJump - 1), ActionJump) THEN jdrop = 1: jpush = 1 'Let go of the JUMP button early (doesn't work!)
    IF jdrop THEN 'When you let go of JUMP
        IF jpush = 0 THEN jpush = 1
        IF plyr& <> CrickMov(0) THEN plyr& = CrickMov(0): CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        CKT% = CKT% + 1
...
ELSEIF slidecount% = 0 AND STICK(ActionDown, JoyMoveOp(1)) = MoveDown THEN
    ...
    END IF
    'IF JoyMoveOp(5) > 0 AND STICK(ActionJump, JoyMoveOp(5)) = MoveJump OR JoyMoveOp(5) = 0 AND STRIG(MoveJump, ActionJump) AND jump% > -2 AND jdrop = 0 AND jpush = 0 THEN
    IF jump% > -2 AND jpush = 0 AND jdrop = 0 AND STRIG(MoveJump, ActionJump) THEN
        'IF jump% = -2 THEN jump% = 0 'Jump off from a ladder/beanstalk.
        IF jump% = 0 THEN _SNDPLAY SEF(1): plyr& = jump&: CKT% = CKB% - _HEIGHT(plyr&): CKR% = CKL% + _WIDTH(plyr&) - 1
        IF jump% < 42 THEN 'Height test, instead of setting the coordinate.
            notop = 0
    ...
    SELECT CASE _KEYHIT 'Works with the keyboard repeat rate, unlike _KEYDOWN.
        ...
        CASE 27 'ESC
            ...
            ELSEIF Sector = 1 THEN 'From the "controls" menu
                ...
                Selector = 2
            END IF
        CASE IS = MovePause OR STRIG(MovePause, ActionPause) 'Press the PAUSE key again
            EXIT DO
    END SELECT
...
ELSEIF cont% > 0 THEN
    ...
    DO
        ...
        xc = cont% * 2
        sax = STICK(xc - 2, 1)
        IF sax > 249 OR sax < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "T1 X:" + STR$(sax): LOOP WHILE INKEY$ = "": EXIT DO
        say = STICK(xc - 1, 1)
        IF say > 249 OR say < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "T1 Y:" + STR$(say): LOOP WHILE INKEY$ = "": EXIT DO
        sbx = STICK(xc - 2, 2)
        IF sbx > 249 OR sbx < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "T2 X:" + STR$(sbx): LOOP WHILE INKEY$ = "": EXIT DO
        sby = STICK(xc - 1, 2)
        IF sby > 249 OR sby < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "T2 Y:" + STR$(sby): LOOP WHILE INKEY$ = "": EXIT DO
        dpx = STICK(xc - 2, 3)
        IF dpx > 249 OR dpx < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "DP X:" + STR$(dpx): LOOP WHILE INKEY$ = "": EXIT DO
        dpy = STICK(xc - 1, 3)
        IF dpy > 249 OR dpy < 6 THEN DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "DP Y:" + STR$(dpy): LOOP WHILE INKEY$ = "": EXIT DO
        IF cont% MOD 2 = 0 THEN bp = STRIG((cb * 4) + 3, cont%) ELSE bp = STRIG((cb * 4) + 1, cont%)
        IF bp THEN
            IF cont% MOD 2 = 0 THEN bn = ((cb * 4) + 3): JoyMoveOp(Highlight - 13) = 0: DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "BT:" + STR$(bn): LOOP WHILE INKEY$ = "": EXIT DO
            IF cont% MOD 2 = 1 THEN bn = ((cb * 4) + 1): JoyMoveOp(Highlight - 13) = 0: DO: LOCATE 1, 1: PRINT "         ": LOCATE 1, 1: PRINT "BT:" + STR$(bn): LOOP WHILE INKEY$ = "": EXIT DO
        END IF
