...
_TITLE "Cricket Kamodon - v0.1.4 Tech Demo"
PRINT "ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป"
PRINT "บ CRICKET KAMODON!         ** GAME ENGINE TECH DEMO **           Version 0.1.4 บ"
...
'First number: number of tiles -1. Second number: # of pixels in each tile -1.
'I'll have to change this engine's tile reading code somewhat, since this
'will force modders to have to recompile the source code, whenever they want
'to change the levels, or the sprites. I mean, I could do tons of DATA lines
'with QB64, but that would make it harder for potential modders.
'DIM tile(52, 63) ' Sets of 8x8 tiles instead of 12x12
'DIM mapdata(110) AS STRING ' For reading from the map data files.
'DIM fglay&(11159) ' Foreground layer
'DIM bglay&(11159) ' Background layer
'DIM splay&(11159)               ' Sprite layer (implementing soon)
...
ckJump& = _SNDOPEN("/media/PHANTOM/BonusOpen.ogg", "VOL,SYNC")
ckSwing1& = _SNDOPEN("/media/PHANTOM/snare.wav", "VOL,SYNC")
ckSwing2& = 0
ckSwing3& = 0
ckExtraLife& = 0
ckHurt& = 0
'ckDeath& = _SNDOPEN("/media/FreeAgent GoFlex Drive/KINGPICCOLO/Users/The Namek/Documents/Stop On A Whammy!.wav", "VOL,SYNC")
ckTimeLow& = _SNDOPEN("/media/PHANTOM/TimeRunningOut.ogg", "VOL,SYNC")
ckTimeUp& = 0

COLOR 15, 0: PRINT "YES!"

ON ERROR GOTO MissingPal
...
END IF
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1BG.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1BG.KMD" FOR INPUT AS #2
...
IF _FILEEXISTS("/media/PHANTOM/QBX/CRICKET/WLDXL1S0.KMD") THEN
    OPEN "/media/PHANTOM/QBX/CRICKET/WLDXL1S0.KMD" FOR INPUT AS #3
    COLOR 7, 0: PRINT "Checking the sprites (1/16)... ";
    DO UNTIL EOF(3)
        FOR n = 0 TO 63
            INPUT #3, drawsomething$
        NEXT n
        INPUT #3, end$ 'Should just say "END", the end-of-line marker
        loopz = loopz + 1
    LOOP
...
CLOSE 1, 2, 3
IF grab = hold THEN
    COLOR 15, 0: PRINT "The BG1 and FG1 layers are working perfectly!"
ELSE
    COLOR 12, 0
...
END IF
'TODO: Move these to the "loading" screen, when we start work on Mushroom 2.
...
COLOR 7, 0: PRINT
PRINT "This tech demo essentially shows off what I think this engine is capable"
PRINT "of. What you're about to see is what I would call my " + CHR$(34) + "Dangerous Dave in"
PRINT "Copyright Infringement" + CHR$(34) + ", in a sense. The only differences would be"
PRINT "Cricket in place of Dangerous Dave, and a clone of world 1-1 through 1-4"
PRINT "from Super Mario Bros. instead of world 1-1 from Super Mario Bros. 3. I"
PRINT "also want to point out that the next demo, and the full release, when it"
PRINT "does come out, will have at least the first level from this demo, but a"
PRINT "completely different level arrangement, with more levels."
PRINT
...
'select screen. So, when I finally start to implement Mushroom 2's map, let's
...
'This will work good, especially when I implement Mushroom 2, and so on
LET FlashColor = 1
LET Highlight = 1
LET TimeIsNow! = TIMER
DO ' EDIT: Unconditional loop, instead of waiting for CHR$(13).
    ...
    IF WhatIsTime! >= .75 THEN
        IF FlashColor THEN LET FlashColor = 0 ELSE LET FlashColor = 1
        IF FlashColor = 0 THEN
            IF Highlight = 1 THEN DRAW "B M120,84": DRAW "C74": Font "MUSHROOM 1"
            IF Highlight = 2 THEN DRAW "B M120,94": DRAW "C74": Font "MUSHROOM 2"
            IF Highlight = 3 THEN DRAW "B M120,104": DRAW "C74": Font "MUSHROOM 3"
            IF Highlight = 4 THEN DRAW "B M120,114": DRAW "C74": Font "MUSHROOM 4"
            IF Highlight = 5 THEN DRAW "B M79,138": DRAW "C74": Font "NEITHER -- EXIT THE DEMO"
        ELSE
            IF Highlight = 1 THEN DRAW "B M120,84": DRAW "C15": Font "MUSHROOM 1"
            IF Highlight = 2 THEN DRAW "B M120,94": DRAW "C15": Font "MUSHROOM 2"
            IF Highlight = 3 THEN DRAW "B M120,104": DRAW "C15": Font "MUSHROOM 3"
            IF Highlight = 4 THEN DRAW "B M120,114": DRAW "C15": Font "MUSHROOM 4"
            IF Highlight = 5 THEN DRAW "B M79,138": DRAW "C15": Font "NEITHER -- EXIT THE DEMO"
        END IF
        LET TimeIsNow! = TIMER
        LET ClockCheck! = TIMER
        LET WhatIsTime! = 0
    ...
    IF _KEYDOWN(18432) THEN ' Up arrow
        ...
        FlashColor = 0
        IF Highlight = 1 THEN DRAW "C74 BM120,84": Font "MUSHROOM 1" ELSE DRAW "C15 BM120,84": Font "MUSHROOM 1"
        IF Highlight = 2 THEN DRAW "C74 BM120,94": Font "MUSHROOM 2" ELSE DRAW "C15 BM120,94": Font "MUSHROOM 2"
        IF Highlight = 3 THEN DRAW "C74 BM120,104": Font "MUSHROOM 3" ELSE DRAW "C15 BM120,104": Font "MUSHROOM 3"
        IF Highlight = 4 THEN DRAW "C74 BM120,114": Font "MUSHROOM 4" ELSE DRAW "C15 BM120,114": Font "MUSHROOM 4"
        IF Highlight = 5 THEN DRAW "C74 BM79,138": Font "NEITHER -- EXIT THE DEMO" ELSE DRAW "C15 BM79,138": Font "NEITHER -- EXIT THE DEMO"
        DO
    ...
    IF _KEYDOWN(13) THEN ' ENTER
        IF Highlight = 1 THEN zone$ = "MUSHROOM 1": EXIT DO
        IF Highlight = 2 THEN zone$ = "MUSHROOM 2": THAT = 0
        IF Highlight = 3 THEN zone$ = "MUSHROOM 3": THAT = 0
        IF Highlight = 4 THEN zone$ = "MUSHROOM 4": THAT = 0
        IF Highlight = 5 THEN _FULLSCREEN _OFF: SYSTEM 'Exit the demo
    END IF
LOOP
...
lc% = (grab / 22) / 340 'How many times does the total go into 340?
rc% = (grab / 22) MOD 340 'Is there a remainder? (One extra loop if there is)

IF lc% < 1 THEN 'If the total is less than 340... run through one loop.
    FOR r% = 0 TO 21
        FOR s% = 0 TO ((grab / 22) - 1)
            ...
            INPUT #2, ns$ 'Go for the background layer, now
            Background1(r%, s%) = VAL(ns$) 'Turn it into an integer, and save
            'TODO: Should I use hexadecimal numbers, instead?
        NEXT s%
    NEXT r%

ELSEIF lc% >= 1 THEN 'If the total is at least 340 or more, do at least one.
    'TODO: How would we make this run through more than one loop?
    FOR r% = 0 TO 21
        FOR s% = 0 TO 339
            INPUT #1, ns$ 'Grab one of the foreground layer sprites
            Foreground1(r%, s%) = VAL(ns$) 'Turn it into an integer, and save
    ...
    'Now, if there was at least 340 or more, but still some left afterward,
    'let's run through one more loop, to get the remaining sprites.

    IF rc% > 0 THEN 'Any sprites left after reading each multiple of 340?
        FOR r% = 0 TO 21
            FOR s% = 0 TO (((grab / 22) - (340 * lc%)) - 1)
                INPUT #1, ns$ 'Grab a sprite
                Foreground1(r%, ((340 * lc%) + s%)) = VAL(ns$) 'Save it
                ns$ = "" 'Clear the variable, because I'm paranoid
                INPUT #2, ns$ 'Grab another sprite
                Background1(r%, ((340 * lc%) + s%)) = VAL(ns$) 'Save it
            NEXT s%
        NEXT r%
    END IF
...
DO
    ...
    FOR screenY = 24 TO 199
        ...
        FOR screenX = 1 TO 320
            ...
            PSET (screenX, screenY), spritedata(Background1(rowpos%, sprpos%), 0, spriteElem)
            IF spritedata(Foreground1(rowpos%, sprpos%), 0, spriteElem) > 0 THEN
                'POKE offset&, spritedata(Foreground1(rowpos%, sprpos%), 0, spriteElem)
                PSET (screenX, screenY), spritedata(Foreground1(rowpos%, sprpos%), 0, spriteElem)
            END IF
            offset& = offset& + 1
    ...
    'Starting position: 152, 168 (this isn't the exact center, but close)
    FOR sr = 0 TO 2
        FOR scy = 0 TO 7
            FOR scx = 0 TO 7
                PSET (152 + scx, (168 + (sr * 8) + scy)), spritedata(4, 0, (scy * 8 + scx))
                PSET (160 + scx, (168 + (sr * 8) + scy)), spritedata(4 + (sr + 1), 0, (scy * 8 + scx))
            NEXT scx
        NEXT scy
    NEXT sr
    ...
    'I can't push two keys simultaneously! Would using _KEYHIT work better?
    kp& = _KEYHIT
    'IF MULTIKEY(75) THEN 'Left arrow key
    IF _KEYDOWN(19200) THEN 'Left arrow key
        'TODO: Use _KEYHIT to check to see if SHIFT, Z or X are also held.
        IF hScroll > 0 THEN
    ...
    ELSEIF _KEYDOWN(19712) THEN 'Right arrow key
        'TODO: Use _KEYHIT to check to see if SHIFT, Z or X are also held.
        IF hScroll < (((grab / 22) * 8) - 320) THEN 'Automatically calculated!
            LET hScroll = hScroll + 1
            LET sprcnt% = sprcnt% + 1
            IF sprcnt% = 8 THEN sprcnt% = 0: sprcur% = sprcur% + 1
            LET sprpos% = sprcur%
        END IF
        'ELSEIF MULTIKEY(29) THEN ' CTRL (original jump key)
    ELSEIF kp& = 100304 THEN 'Pressing LEFT SHIFT (default RUN key) by itself
        'TODO: Try combining the keycode with the Z, X or L/R arrow keys?
    ...
    ELSEIF kp& = 122 THEN 'Pressing Z (default JUMP key) by itself
        _SNDPLAY (ckSwing1&) 'Just for testing purposes.
    ELSEIF kp& = 120 THEN 'Pressing X (default ATTACK key) by itself
        _SNDPLAY (ckSwing1&) 'Just for testing purposes.
        'ELSEIF MULTIKEY(1) THEN END ' ESC ends our fancy little simulation.
    ELSEIF kp& = 27 THEN END 'ESC ends our fancy little simulation.
    END IF
...
DRAW "B M38,84"
Font "SIDESCROLLING ENGINE BY AARON SEVERN"
DRAW "B M6,94"
